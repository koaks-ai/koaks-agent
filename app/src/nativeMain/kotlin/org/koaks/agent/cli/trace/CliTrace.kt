...
import org.koaks.agent.cli.platform.PosixTraceWriter
...
internal class CliTrace(
    ...
    override fun eventReceived(event: AgentEvent) {
        ...
        log("agent.event.received", "turn=$turn index=$currentAgentEventIndex type=${event.kind()}${event.sizeFields()}")
        when (event) {
            ...
            is AgentEvent.ReasoningDelta -> {
                if (!reasoningRunActive) log("agent.reasoning.first", "turn=$turn step=$currentStep chars=${event.text.length}")
    ...
    override fun eventRendered(event: AgentEvent) {
        ...
        if (event is AgentEvent.ToolResult) log("tool.result.rendered", "turn=$turn step=$currentStep id=${event.callId.safe()}")
    }
    ...
    override fun collectorFailed(error: Throwable) =
        log("collector.failed", "turn=$turn error=${error::class.simpleName} message_chars=${error.message?.length ?: 0}")
    ...
    override fun onStep(state: AgentState) {
        ...
        textRunActive = false
        reasoningRunActive = false
        log("model.request.start", "turn=$turn step=$currentStep messages=${state.messages.size} global_step=${state.globalStep}")
    }
    ...
    override fun onModelEvent(event: ModelEvent) {
        ...
        log("model.event", "turn=$turn step=$currentStep index=$modelEventsInStep type=${event.kind()}${event.sizeFields()}")
        when (event) {
    ...
    internal companion object {
        ...
        fun open(environment: Environment): CliTrace {
            ...
            return CliTrace(PosixTraceWriter.open(path))
        }
