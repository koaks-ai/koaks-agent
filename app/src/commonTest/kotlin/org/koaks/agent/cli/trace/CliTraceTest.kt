...
class CliTraceTest {
    ...
    fun recordsToolResultRenderingWithoutContent() {
        ...
        trace.onStep(AgentState(emptyList()))
        trace.onModelEvent(ModelEvent.TextDelta("secret model text"))
        ...
        assertContains(content, "event=model.request.start")
        assertContains(content, "event=model.event")
        ...
        assertFalse(content.contains("secret model text"))
    }
