...
import org.koaks.agent.tui.render.TextUtil
import org.koaks.agent.tui.render.Theme
import org.koaks.agent.tui.state.MAX_INPUT_TEXT_ROWS
import org.koaks.agent.tui.state.PANEL_WIDTH
import org.koaks.agent.tui.state.TerminalLayout

/** Renders the small framed blocks used by the terminal frontend. */
internal object InputBox {
    fun editorTextWidth(): Int = inputTextWidth(PANEL_WIDTH)

    fun editorTextWidth(layout: TerminalLayout): Int = inputTextWidth(if (layout.fixedInput) inputBlockWidth(layout) else PANEL_WIDTH)

    fun editorTextRows(
        snapshot: LineEditorSnapshot,
        layout: TerminalLayout,
    ): Int = fixedInputDisplay(snapshot, inputBlockWidth(layout)).rows.size

    fun renderStaticStart(
        output: Output,
        theme: Theme,
        commandMenuRows: Int,
    ) {
        output.write(Ansi.BLINKING_BAR_CURSOR)
        output.writeLine()
        repeat(commandMenuRows.coerceAtLeast(0)) { output.writeLine() }
        output.writeLine(inputBlockPaddingRow(theme, PANEL_WIDTH, top = true, BlockAccent.USER_INPUT))
        output.write(inputBlockRow("", theme, PANEL_WIDTH, BlockAccent.USER_INPUT))
        output.write(Ansi.cursorColumn(INPUT_TEXT_COLUMN))
    }
    ...
    fun renderStaticEnd(
        output: Output,
        theme: Theme,
        inputWasEchoed: Boolean,
    ) {
        if (!inputWasEchoed) output.writeLine()
        output.writeLine(inputBlockPaddingRow(theme, PANEL_WIDTH, top = false, BlockAccent.USER_INPUT))
        output.write(Ansi.RESET_CURSOR_STYLE)
    }
    ...
    fun renderFixed(
        ...
        drawFixedInputBox(output, layout, theme, snapshot = null, FixedInputDisplay.empty(), emptyList())
    ...
    fun renderFixedEditor(
        ...
        val menuLines =
            commandMenuLines(
                snapshot = snapshot,
                theme = theme,
                width = layout.columns - INPUT_BORDER_WIDTH - COMMAND_MENU_GAP_WIDTH,
                capacity = layout.commandMenuRows,
            )
    ...
    fun renderContentBlock(
        ...
        output.writeLine(inputBlockPaddingRow(theme, safeWidth, top = true, accent))
        lines.forEach { line ->
            wrappedTextRows(line, inputTextWidth(safeWidth)).forEach { row ->
                output.writeLine(inputBlockRow(line.substring(row.start, row.end), theme, safeWidth, accent))
            }
        }
        output.writeLine(inputBlockPaddingRow(theme, safeWidth, top = false, accent))
    }
    ...
    private fun drawFixedInputBox(
        ...
        output.write(
            "${Ansi.cursor(boxTopRow, 1)}${Ansi.CLEAR_LINE}" +
                inputBlockPaddingRow(theme, width, top = true, BlockAccent.USER_INPUT),
        )
        display.rows.forEachIndexed { index, row ->
            val content =
                if (snapshot == null) {
                    inputBlockRow("", theme, width, BlockAccent.USER_INPUT)
                } else {
                    fixedInputContent(snapshot, row, theme, width)
                }
            output.write("${Ansi.cursor(boxTopRow + index + 1, 1)}${Ansi.CLEAR_LINE}$content")
        }
        output.write(
            "${Ansi.cursor(layout.inputBottomRow, 1)}${Ansi.CLEAR_LINE}" +
                inputBlockPaddingRow(theme, width, top = false, BlockAccent.USER_INPUT),
        )
    }

    private fun fixedInputContent(
        snapshot: LineEditorSnapshot,
        row: WrappedTextRow,
        theme: Theme,
        width: Int,
        ...
        val content =
            when {
                commandEnd == null || row.start >= commandEnd -> displayText.substring(row.start, row.end)
                row.end <= commandEnd -> theme.inputCommand(displayText.substring(row.start, row.end))
                else ->
                    theme.inputCommand(displayText.substring(row.start, commandEnd)) +
                        displayText.substring(commandEnd, row.end)
            }
        return inputBlockRow(content, theme, width, BlockAccent.USER_INPUT)
    }
    ...
    private fun inputBlockRow(
        ...
        val side = if (accent == BlockAccent.WELCOME) theme.welcomeSide() else theme.inputSide()
        val body = "  $content"
    ...
    private fun inputBlockPaddingRow(
        ...
        val border = if (top) "╻" else "╹"
        val fill = if (top) "▄" else "▀"
        val side =
            if (accent == BlockAccent.WELCOME) {
                theme.welcomePaddingSide(border)
            } else {
                theme.inputPaddingSide(border)
            }
    ...
    private fun commandMenuLines(
        ...
        val start =
            when {
                ...
                selected == null || selected < capacity -> 0
        ...
        return suggestions.drop(start).take(capacity).mapIndexed { offset, suggestion ->
            ...
            TextUtil.truncateVisible("$marker $command  ${theme.dim(suggestion.description)}", width)
        }
    }

    private fun staticInputContent(
        snapshot: LineEditorSnapshot,
        theme: Theme,
        width: Int,
    ): String {
        ...
        val content =
            when {
                commandEnd == null || viewport.start >= commandEnd -> displayText.substring(viewport.start, viewport.end)
                viewport.end <= commandEnd -> theme.inputCommand(displayText.substring(viewport.start, viewport.end))
                else ->
                    theme.inputCommand(displayText.substring(viewport.start, commandEnd)) +
                        displayText.substring(commandEnd, viewport.end)
            }
        return inputBlockRow(content, theme, width, BlockAccent.USER_INPUT)
    }

    private fun inputCursorColumn(
        snapshot: LineEditorSnapshot,
        width: Int,
    ): Int {
    ...
    private fun inputViewport(
        ...
        var end = safeCursor
        var visibleWidth = 0
        val preferredLeftWidth = width.coerceAtLeast(1) / 2
        ...
        while (end < lineEnd) {
            ...
            if (characterWidth > 0 && visibleWidth + characterWidth > width) break
            end = next
            visibleWidth += characterWidth
        }
        while (start > lineStart) {
        ...
        return InputViewport(start, end, TextUtil.visibleWidth(text.substring(start, safeCursor)))
    }

    private fun inputTextWidth(width: Int): Int = (width - INPUT_BORDER_WIDTH - INPUT_TEXT_PADDING_WIDTH).coerceAtLeast(1)

    private fun wrappedTextRows(
        text: String,
        width: Int,
    ): List<WrappedTextRow> {
        ...
        while (index < text.length) {
            if (text[index] == '\n') {
                rows += WrappedTextRow(start, index)
                start = index + 1
                rowWidth = 0
                index++
                continue
            ...
            val charWidth = TextUtil.visibleWidth(text.substring(index, next))
            if (charWidth > 0 && rowWidth > 0 && rowWidth + charWidth > safeWidth) {
                rows += WrappedTextRow(start, index)
                start = index
                rowWidth = 0
            }
        ...
        rows += WrappedTextRow(start, text.length)
        return rows
    }

    private fun fixedInputDisplay(
        snapshot: LineEditorSnapshot,
        width: Int,
    ): FixedInputDisplay {
        val displayText = snapshot.displayText
    ...
    private fun inputBlockWidth(layout: TerminalLayout): Int = (layout.columns - 1).coerceAtLeast(MIN_INPUT_BLOCK_WIDTH)

    private data class WrappedTextRow(
        val start: Int,
        val end: Int,
    )
    ...
    private data class FixedInputDisplay(
        ...
        fun boxTopRow(layout: TerminalLayout): Int = layout.inputBottomRow - rows.size - 1

        fun cursorRow(layout: TerminalLayout): Int = boxTopRow(layout) + cursorVisibleRow + 1

        companion object {
            fun empty(): FixedInputDisplay =
                FixedInputDisplay(
