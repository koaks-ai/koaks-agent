...
internal class LineEditor(private val request: LineReadRequest) {
    ...
    private fun moveCursorVertically(delta: Int): Boolean {
        ...
        val column = preferredVisualColumn
    ...
    private fun cursorIndexAtColumn(row: EditorVisualRow, column: Int): Int {
        var index = row.start
        var visibleWidth = 0
        while (index < row.end) {
            val next = nextCharacterIndex(text, index)
...
private data class EditorVisualRow(val start: Int, val end: Int)

internal sealed interface TerminalKey {
    data class Text(val value: String) : TerminalKey
    data class Paste(val value: String) : TerminalKey
...
internal fun decodeCsiEnterKey(sequence: String): TerminalKey? {
    ...
    val modifier = when (sequence.last()) {
        'u' -> {
