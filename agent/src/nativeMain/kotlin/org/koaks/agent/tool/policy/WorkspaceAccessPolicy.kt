...
public class WorkspaceAccessPolicy public constructor(
    root: String = NativeFileSystem.workingDirectory(),
    public val maxReadChars: Int = 100_000,
    public val maxWriteChars: Int = 1_000_000,
) {
    private val canonicalRoot =
        NativeFileSystem.canonicalPath(root)?.normalizeSeparators()
    ...
    private fun resolve(
        ...
        if (mustExist && !NativeFileSystem.fileExists(lexical)) fail("unable to resolve path: $path")

        val canonical =
            NativeFileSystem.canonicalPath(lexical)
    ...
    private fun canonicalizeNewPath(path: String): String {
        ...
        val canonicalParent =
            NativeFileSystem.canonicalPath(parent)
