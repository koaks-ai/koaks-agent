    ...
    private fun credential(settings: EffectiveAgentSettings): String {
        ...
        val reference =
            settings.credentialRef
                ?: throw AgentSetupException(SetupFailure.MissingCredentialReference(settings.model.provider))
        return credentials.resolve(reference)
            ?: throw AgentSetupException(SetupFailure.CredentialUnavailable(settings.model.provider, reference))
    }
