...
internal actual object BashCommandLine {
    ...
    actual fun execute(
        ...
        val process = runProcess(commandLine, captureLimit, HOST_ERROR_CAPTURE_BYTES, timeoutMillis)
...
private fun powerShellScript(
    marker: String,
    encodedUserCommand: String,
): String =
    """
    ${'$'}ProgressPreference = 'SilentlyContinue'
    ${'$'}koaksAllowedEnvironment = @(
        'PATH', 'PATHEXT', 'SystemRoot', 'COMSPEC', 'TEMP', 'TMP', 'USERPROFILE',
        'HOMEDRIVE', 'HOMEPATH', 'APPDATA', 'LOCALAPPDATA', 'PROGRAMDATA',
        'ProgramFiles', 'ProgramFiles(x86)', 'JAVA_HOME'
    )
    Get-ChildItem Env: | Where-Object { ${'$'}_.Name -notin ${'$'}koaksAllowedEnvironment } |
...
private fun runProcess(
    ...
    memScoped {
        val security =
            alloc<SECURITY_ATTRIBUTES>().apply {
        ...
        try {
            while (true) {
                ...
                if (!exited && started.elapsedNow().inWholeMilliseconds >= timeoutMillis.coerceAtLeast(1)) {
                    timedOut = true
                    TerminateProcess(processInfo.hProcess, TIMEOUT_EXIT_CODE.toUInt())
                }
            ...
            WindowsProcessOutput(
                status = if (timedOut) TIMEOUT_EXIT_CODE else exitCode.value.toInt(),
                stdout = stdout.bytes(),
...
private data class WindowsProcessOutput(
    ...
    companion object {
        fun launchFailure(error: Int): WindowsProcessOutput = WindowsProcessOutput(1, byteArrayOf(), 0, false, byteArrayOf(), false, error)
    }
...
private fun String.containsCliXml(): Boolean = contains("#< CLIXML") || contains("<Objs Version=")
