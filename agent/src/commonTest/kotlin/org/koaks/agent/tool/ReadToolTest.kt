...
import org.koaks.agent.tool.fs.ReadInput
...
class ReadToolTest {
    @Test
    fun readsSmallFileWithLineNumbers() =
        runBlocking {
    ...
    @Test
    fun summarizesLargeFileWithoutExplicitWindow() =
        runBlocking {
...
private suspend fun withTempTextFile(
    text: String,
    block: suspend (String) -> Unit,
) {
    val path = ".koaks-read-tool-test-${Random.nextInt(0, Int.MAX_VALUE)}.txt"
    val write = PlatformFileSystem.writeWholeFile(path, text)
    check(write.error == null) { write.error ?: "unable to create temp file" }
