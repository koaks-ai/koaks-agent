...
import org.koaks.agent.definition.SubagentDefinitionProvider
...
public fun ToolScope.registerBuiltinCliTools(
    subagentDefinitions: SubagentDefinitionProvider,
    workspacePolicy: WorkspaceAccessPolicy = WorkspaceAccessPolicy(),
    processPolicy: ProcessPolicy = ProcessPolicy(),
) {
    tool(BashTool(processPolicy))
    tool(ReadTool(workspacePolicy))
    tool(WriteTool(workspacePolicy))
    tool(EditTool(workspacePolicy))
    tool(SubagentTool(subagentDefinitions))
}

public fun ToolScope.registerSubagentBuiltinCliTools(
    workspacePolicy: WorkspaceAccessPolicy = WorkspaceAccessPolicy(),
    processPolicy: ProcessPolicy = ProcessPolicy(),
) {
    tool(BashTool(processPolicy))
        ...
        executeToolSafely(name) {
            ...
            val result =
                withRuntimeResource("local-process", AccessMode.WRITE) {
                    NativeCliIo.runBash(command, policy.maxOutputChars, policy.timeoutMillis)
                }
        ...
        executeToolSafely(name) {
            ...
            val scan =
                withRuntimeResource("file:$path", AccessMode.READ) {
                    NativeCliIo.readTextWindow(path, offset, effectiveLimit, maxChars)
                }
        ...
        executeToolSafely(name) {
            ...
            val write =
                withRuntimeResource("file:$path", AccessMode.WRITE) {
                    NativeCliIo.writeWholeFile(path, input.content)
                }
...
internal class EditTool(
    ...
    override suspend fun execute(input: EditInput): String =
        executeToolSafely(name) {
            ...
            val read =
                withRuntimeResource("file:$path", AccessMode.READ) {
                    NativeCliIo.readWholeFile(path, minOf(MAX_EDIT_FILE_BYTES, policy.maxWriteChars.toLong()))
                }
            ...
            val write =
                withRuntimeResource("file:$path", AccessMode.WRITE) {
                    NativeCliIo.writeWholeFile(path, toWrite)
                }
            if (write.error != null) return@executeToolSafely "Error: ${write.error}"
...
private fun displayName(path: String): String = path.substringAfterLast('/').substringAfterLast('\\').ifEmpty { path }
