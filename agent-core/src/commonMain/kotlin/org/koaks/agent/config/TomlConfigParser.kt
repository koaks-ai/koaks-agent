...
private class RootBuilder {
    ...
    fun apply(
        ...
        when (key) {
            "schema_version" -> schemaVersion = parsePositiveIntValue(key, value, sourceName, lineNumber)
...
private class ProviderBuilder {
    ...
    fun apply(
        ...
        when (key) {
            ...
            "credential_source" ->
                credentialSource =
                    CredentialSource.parse(
                        parseNonBlankString(key, value, sourceName, lineNumber),
                    )
    ...
    fun build(): FileProviderConfig {
        if ((credentialSource == null) != (credentialName == null)) {
            throw CliException("credential_source and credential_name must be configured together.")
        }
        if (apiKey != null && credentialSource != null) {
            throw CliException("Configure either api_key or credential_source/credential_name, not both.")
        }
...
private fun parseProviderString(
    value: String,
    sourceName: String,
    lineNumber: Int,
): Provider = parseProvider(parseStringValue(value, sourceName, lineNumber), sourceName, lineNumber)
...
private fun parseNonBlankString(
    key: String,
    value: String,
    sourceName: String,
    lineNumber: Int,
...
private fun parsePositiveIntValue(
    key: String,
    value: String,
    sourceName: String,
    lineNumber: Int,
    ...
    val parsed =
        parseStringValue(value, sourceName, lineNumber).toIntOrNull()
...
private fun configError(
    ...
    lineNumber: Int,
    message: String,
): Nothing = throw CliException("$sourceName:$lineNumber: $message")
