...
public data class AgentConfig public constructor(
    ...
    public val showReasoning: Boolean,
    public val skillPaths: List<String>,
    public val skills: List<String>,
    public val providerProfiles: Map<Provider, ProviderProfile>,
...
public fun AgentConfig.profileFor(provider: Provider): ProviderProfile =
...
public fun AgentConfig.availableProviders(): List<Provider> = configuredProviders.ifEmpty { Provider.entries.toList() }

public typealias ResolvedAppConfig = AgentConfig

public data class SessionPreferences public constructor(
...
public fun ResolvedAppConfig.withPreferences(preferences: SessionPreferences): ResolvedAppConfig {
    val provider = preferences.provider ?: provider
    val profile = profileFor(provider)
    return copy(
        provider = provider,
        baseUrl = profile.baseUrl,
        credentialRef = profile.credentialRef,
        modelName = preferences.modelName ?: if (provider == this.provider) modelName else profile.defaultModel,
        showReasoning = preferences.showReasoning ?: showReasoning,
        skills = preferences.skills ?: skills,
