...
internal class Theme(
    ...
    fun inputSide(): String = if (enabled) "${Ansi.USER_INPUT_BORDER}┃${Ansi.RESET}" else "┃"

    fun inputPaddingSide(text: String): String = if (enabled) "${Ansi.USER_INPUT_BORDER}$text${Ansi.RESET}" else text

    fun welcomeSide(): String = if (enabled) "${Ansi.WELCOME_BORDER}┃${Ansi.RESET}" else "┃"

    fun welcomePaddingSide(text: String): String = if (enabled) "${Ansi.WELCOME_BORDER}$text${Ansi.RESET}" else text
    ...
    private fun color(
        code: String,
        text: String,
    ): String = if (enabled) "$code$text${Ansi.RESET}" else text
}
