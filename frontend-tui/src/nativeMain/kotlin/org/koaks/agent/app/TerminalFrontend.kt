...
import org.koaks.agent.app.command.CommandResult
import org.koaks.agent.app.command.MessageTone
import org.koaks.agent.config.CliException
import org.koaks.agent.config.toBooleanFlagOrFalse
...
import org.koaks.agent.tui.DEFAULT_TERM_ROWS
...
public class TerminalFrontend internal constructor(
    ...
    public constructor(
        ...
        trace: FrontendTrace = NoopFrontendTrace,
        environment: Environment = PosixEnvironment,
    ...
    private var state = UiState(session.config)
    ...
    public suspend fun run() {
        ...
        try {
            ...
            while (!state.exitRequested) {
                ...
                when (val command = commands.dispatch(commandInput, session.config)) {
                    CommandResult.Exit -> {
                    ...
                    is CommandResult.Update -> {
                        session.updatePreferences(command.transform)
                        state = state.copy(config = session.config)
                        printCommandMessage(command.message.text, command.message.tone, theme, contentOutput)
                        continue
                    }
                    null -> Unit
                }
                ...
                val events =
                    try {
                        ...
                        contentOutput.writeLine(theme.error("[error] ${error.message}"))
                        contentOutput.flush()
                        dispatch(UiAction.RunFailed(error.message ?: "Unable to start run."))
                        continue
                    }

                if (!layout.fixedInput) contentOutput.writeLine()
                val printer = EventPrinter(session.config.showReasoning, contentOutput, theme, trace)
