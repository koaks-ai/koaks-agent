...
import org.koaks.agent.tui.TerminalMarkdownRenderer
...
internal class EventPrinter(
    ...
    private val trace: EventRenderTrace? = null,
) {
    ...
    fun print(event: AgentEvent) {
        when (event) {
            ...
            is AgentEvent.Failed -> {
                ...
                event.error.cause?.let { cause ->
                    val stack = cause.stackTraceToString().trimEnd()
                    if (stack.isNotEmpty() && !event.error.message.contains(stack)) {
                        output.writeLine(theme.error(stack))
                    }
                }
                flushOutput()
            }
    ...
    private fun printToolCall(event: AgentEvent.ToolCallRequested) {
        ...
        pendingToolCalls[event.call.id] =
            PendingToolCall(
                line = line,
